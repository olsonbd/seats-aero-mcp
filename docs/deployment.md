...
The Docker image includes a Node.js-based health check and curl for hosts that supply their own HTTP probes. If using a platform-managed probe, point it at `/health` on the configured internal HTTP port.
